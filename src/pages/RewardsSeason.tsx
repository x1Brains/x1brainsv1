            ...
            {(()=>{
              ...
              return stats.map((s,i)=>(
                <div key={i} style={{padding:isMobile?'14px 10px':'18px 16px',textAlign:'center',borderRight:(!isMobile&&i<stats.length-1)?'1px solid rgba(120,60,255,.06)':'none',borderBottom:(isMobile&&i<2)?'1px solid rgba(120,60,255,.06)':'none'}}>
                  ...
                  <div style={{fontFamily:'Orbitron,monospace',fontSize:isMobile?13:17,fontWeight:900,letterSpacing:1,color:s.c,textShadow:`0 0 10px ${s.c}55`,animation:s.l==='TIME LEFT'&&isLive?'rw-cd 2s ease infinite':undefined}}>{s.v}</div>
                </div>));
            })()}
          </div>
        </div>
              ...
              <div style={{position:'relative',zIndex:1,padding:isMobile?'20px 14px':'30px 28px'}}>
                {lastLog&&lastLog.winners&&lastLog.winners.length>0?(
                  <div>
                    ...
                    <div style={{display:'flex',flexDirection:isMobile?'column':'row',gap:isMobile?10:12,alignItems:isMobile?'stretch':'flex-end',marginBottom:20}}>
                      {(isMobile
                        ? [lastLog.winners.find((w:any)=>w.place===0),lastLog.winners.find((w:any)=>w.place===1),lastLog.winners.find((w:any)=>w.place===2)]
                        : [lastLog.winners.find((w:any)=>w.place===1),lastLog.winners.find((w:any)=>w.place===0),lastLog.winners.find((w:any)=>w.place===2)]
                      ).filter(Boolean).map((wn:any)=>{
                        const rank=wn.place+1;const cfg=POD_CFG[wn.place];const isTop=wn.place===0;const imgSize=isMobile?(isTop?72:56):(isTop?100:80);
                        return(
                          <div key={wn.place} style={{flex:isMobile?'none':1,position:'relative',overflow:'hidden',background:'linear-gradient(160deg,#06040e,#08060f)',border:`1px solid ${cfg.border}${isTop?'88':'55'}`,borderTop:`3px solid ${cfg.border}`,borderRadius:12,padding:isMobile?'14px 14px':'16px 14px',minHeight:isMobile?'auto':(isTop?280:240),display:'flex',flexDirection:isMobile?'row':'column',alignItems:'center',gap:isMobile?14:0}}>
                            <GridBg />
                            {/* Left side on mobile: rank + avatar */}
                            <div style={{display:'flex',flexDirection:'column',alignItems:'center',flexShrink:0,position:'relative',zIndex:2}}>
                              ...
                              <div style={{position:'relative',marginBottom:isMobile?0:8}}>
                            ...
                            {/* Right side on mobile: stats; center on desktop */}
                            <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:isMobile?'flex-start':'center',width:isMobile?'auto':'100%',position:'relative',zIndex:2,minWidth:0}}>
                              <div style={{fontFamily:'monospace',fontSize:isMobile?11:13,color:cfg.label,marginBottom:3}}>{short(wn.address)}</div>
                              <button onClick={()=>navigator.clipboard.writeText(wn.address)} style={{marginBottom:6,background:`${cfg.border}10`,border:`1px solid ${cfg.border}30`,color:cfg.label,padding:'4px 10px',fontFamily:'Orbitron,monospace',fontSize:isMobile?7:8,borderRadius:5,cursor:'pointer',letterSpacing:1}}>📋 COPY</button>
                              <div style={{display:'flex',flexDirection:'column',gap:5,width:'100%',marginTop:isMobile?0:'auto'}}>
