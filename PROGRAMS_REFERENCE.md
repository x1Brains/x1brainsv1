...
- **PoolRecord** `[b"pool_record", pool_address]` (**282** on chain = `space = 8 + 274`; the struct fills only 268, leaving 6 bytes of slack): `pool_address@8`, `lp_mint@40`, `token_a_mint@72`, `token_b_mint@104`, `sym_a@136[12]`, `sym_b@148[12]`, `burn_bps@160`, `lp_burned@162`, `lp_treasury@170`, `lp_user_a@178`, `lp_user_b@186`, `creator_a@194`, `creator_b@226`, `usd_val@258`, `created_at@266`, **`seeded@274`** (1=admin‑seeded xDEX pool, 0=marketplace‑matched), `bump@275`. *(This is what x1brainsv2's `/charts` reads; matched pools have `seeded=0`.)*
  > ⚠️ **Corrected 2026-08-17.** Everything from `burn_bps` onward was previously listed 8 bytes early (the running total skipped `sym_b`), which put `seeded` at 266 and `bump` at 267 — implying a 268-byte account. Verified against `state.rs` and all 10 live records: the 5 admin-seeded ones have `d[274]=1` and `d[281]=0`, so a reader checking byte 281 sees slack and never skips them.
